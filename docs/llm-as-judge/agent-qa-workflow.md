...
当前自动化 smoke 使用 `openathor-judge-smoke` 生成 evidence package。smoke 默认不调用模型，而是检查真实 fixture replay、CLI 输出、writes/warnings 和 file changes 是否足以交给 judge；真实 Pi Agent transcript 和最终回复可以通过 `--scenario <name> --operator-transcript <path> --agent-final-response <path>` 附加到单个 evidence package，LLM judge scores 后续填入同一证据格式。
