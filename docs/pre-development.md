...
- Slice 3 outline show/impact/insert/move/archive 结构编辑最小闭环
- Slice 4 search text/search related 确定性检索
- LLM-as-judge evidence package smoke 入口

## 推荐下一步

下一步应继续推进 split/merge/replan 等结构编辑命令、向量语义检索的可选派生能力，以及真实 Pi Agent transcript 和 LLM judge scores 的本地/手动评估证据保存流程。
