...
- Slice 2 style analyze/profile show/profile apply、style check 和 style revise 风格维护入口
- Slice 2 confirmed active style guidance 进入 context、draft/review/revise proposal，并排除 pending profile
- Slice 3 outline show/impact/insert/move/split/merge/replan/archive 结构编辑最小闭环
- Slice 4 search text/search related/search semantic 检索入口
- LLM-as-judge evidence package smoke 和本地真实 judge scores attachment 入口

## 推荐下一步

下一步应继续推进更复杂的 replan 场景，以及更真实的 Pi Agent 端到端场景集。
