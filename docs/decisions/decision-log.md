...
## 2026-06-05: 结构编辑主线补齐到 proposal/confirmed split

Slice 3 继续补齐以下能力：

- `openathor outline split --confirm --base-hash`
- `openathor outline merge`
- `openathor outline replan`

当前行为：

- `split` 默认仍是 proposal；只有传入 `--confirm --base-hash` 才写入。
- confirmed split 修改目标正文为前半段，创建后一段新正文，更新 `outline/chapters.yaml` 和 `.openathor/manuscript.index.yaml`，并写入 `runs/run_*_outline_split.json`。
- confirmed split 会顺延后续章节 display order。
- confirmed split 不删除、移动或重命名其他正文文件，不修改 confirmed canon。
- `merge` 当前只生成合章 proposal，不写文件。
- `replan` 当前只生成重规划 proposal，不写文件。

原因：

- 拆章已具备可明确定位的文本边界和 base hash 保护，可以作为受控 confirmed write。
- 合章和重规划会影响更多上下文和引用，先保持 proposal-only，等待更完整的影响分析和确认交互。

验证：

- `fixtures/slice-3/outline-merge`
- `fixtures/slice-3/outline-replan`
- `fixtures/slice-3/outline-split-confirm`

## 2026-06-05: 可选派生向量检索已落地

Slice 4 增加可选派生向量索引和语义检索入口。
...
- `--vector` 额外生成 `.openathor/vector/index.json`。
- vector index 是可删除、可重建的派生数据，不保存唯一用户内容。
- 当前 embedding 方法是本地 deterministic hash embedding，不调用外部模型服务。
- `search semantic` 只读 vector index；索引不存在时返回 `OA_VECTOR_INDEX_NOT_FOUND`。

验证：

- `fixtures/slice-4/search-semantic`

## 2026-06-05: 本地真实 LLM Judge 分数可附加到 evidence package

`openathor-judge-smoke` 支持把本地 judge scores JSON 附加到单个 evidence package。

已实现：

- `--judge-scores <path>`
- score 维度校验
- verdict、blocking failures、judge model 和 notes 保存
- 附加分数后从 `missing_evidence` 移除 `llm_judge_scores`

当前验证：

- 本地使用 Pi Agent `0.78.0` 和 `jdcloud-anthropic/GLM-5` 对 `draft-confirm-write` evidence package 进行真实 judge 评分。
- 评分保存到 `evals/manual/draft-confirm-write-scores.json`。
