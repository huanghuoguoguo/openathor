...
type OutlineReplanChapterInput = {
  id: string | null;
  title: string;
...
type OutlineReplanPackage = {
  chapters: OutlineReplanChapterInput[];
};

type OutlineReplanPlan = {
...
export async function runOutlineReplan(
  ...
  const replanPackage = packagePath
    ? await readOutlineReplanPackage(projectRoot, packagePath)
    : null;
  const plan = replanPackage
    ? buildOutlineReplanPlan(replanPackage, inspection, from)
    : null;
  const stamp = runStamp();
...
function replanConfirmedWrites(runRelPath: string): EnvelopeWrite[] {
  return [
    {
      path: "outline/chapters.yaml",
      change_type: "modified",
...
async function readOutlineReplanPackage(
  projectRoot: string,
  safeRelPath: string,
...
function normalizeOutlineReplanPackagePath(relPath: string): string {
...
function buildOutlineReplanPlan(
  replanPackage: OutlineReplanPackage,
  inspection: Awaited<ReturnType<typeof inspectProject>>,
  from: ResolvedOutlineChapter,
): OutlineReplanPlan {
  const preservedBefore = inspection.chapters.chapters
    .filter((chapter) => chapter.display_order < from.display_order)
    .sort((a, b) => a.display_order - b.display_order);
  const replacedChapters = inspection.chapters.chapters
    .filter((chapter) => chapter.display_order >= from.display_order)
    .sort((a, b) => a.display_order - b.display_order);
  const archivedIndexChapters = inspection.manuscriptIndex.chapters.filter(
  ...
  const existingIds = new Set([
    ...inspection.chapters.chapters.map((chapter) => chapter.id),
    ...inspection.manuscriptIndex.chapters.map((chapter) => chapter.id),
  ]);
...
function validateConfirmedReplanSafe(
  plan: OutlineReplanPlan,
  manuscriptIndex: ManuscriptIndex,
): void {
