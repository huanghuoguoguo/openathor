...
- 支持多角色审稿包，让 Pi Agent 分角色检查连续性、结构、风格和读者体验
...
```bash
curl -fsSL https://github.com/huanghuoguoguo/openathor/releases/latest/download/install.sh | sh
openathor --version
```

安装脚本会从 GitHub Releases 下载 `openathor.tar.gz`，校验 SHA256，安装到 `~/.openathor/current`，并把命令链接到 `~/.local/bin`：
...
`npm test` 会运行 TypeScript 类型检查、schema 校验、构建、package/bin smoke、fixture checker 自测、44 个 deterministic fixture 回归，以及不调用外部模型的 judge smoke。
...
对已经纳入 OpenAthor 的章节，可以让 Pi Agent 基于同一份上下文分角色审稿：

```bash
openathor review chapter 1 --task "从连续性、结构、风格和读者体验角度审稿" --multi-agent --json
```

CLI 会返回 `review_pack`，默认包含：

- `context-scout`：查找相关前文、notes、assets 和未解决伏笔
- `continuity-reviewer`：检查 canon、时间线、人物状态和伏笔连续性
- `outline-planner`：检查章节目标、场景顺序、节奏和信息释放
- `style-editor`：检查 confirmed style profile 和文风漂移
- `reader-qa`：从读者角度找困惑、动机缺口和情绪断点
- `qa-judge`：检查合并后的审稿证据质量和安全边界

也可以只指定部分角色：

```bash
openathor review chapter 1 --task "只复核连续性和文风" --review-role continuity-reviewer --review-role style-editor --json
```

多角色审稿包仍然是 proposal：它只生成 role pack、findings schema、merge policy 和 review notes，不直接修改正文、confirmed canon 或 outline。

### 运行真实项目回归样例

```bash
npm run fixture -- fixtures/slice-4/adopt-30-chapters
npm run fixture -- fixtures/slice-2/multi-agent-review
npm run fixture -- fixtures/slice-4/style-guided-writing-loop
npm run fixture -- fixtures/slice-4/replan-draft-asset-continuity
```

这些样例覆盖 30 章接管、多角色审稿包、风格画像写作闭环、重规划后继续写作和创作资产连续性。
...
`review --multi-agent` 不把审稿结果藏在一次模型回复里，而是先生成确定性 role pack。每个角色必须基于同一份 context 输出结构化 findings，主 Operator Agent 再合并、去重、分级，并保留证据不足或角色分歧。
...
openathor draft
openathor review
openathor review chapter <id-or-order> --multi-agent
openathor revise
...
- Slice 2：写作上下文、draft/review/revise proposal、多角色审稿包、确认写章、hash 冲突、canon 冲突
...
GitHub Actions 会在 PR 和 main 上运行文档健康检查、类型检查、schema 校验、构建、package/bin smoke、fixture checker 自测、fixture 回归和 judge smoke。
...
- OpenAthor CLI 不直接调度真实 sub-agent；`review --multi-agent` 只生成确定性审稿包，由 Pi Agent 或运行时按角色执行。
