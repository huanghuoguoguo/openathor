...
当前实现覆盖 Slice 1 协议内核、Slice 2 的只读 `context` 入口、plan/draft/review/revise/canon sync 的 proposal 入口、确认后的“下一章”安全写入、带 `--base-hash` 冲突保护的已有章节确认改写、结构编辑 show/impact/insert/move/split/merge/replan/archive 最小闭环、文本/相关/semantic 检索，以及 `openathor-judge-smoke` evidence package 自动化。CLI 不调用模型写正文，不做 sub-agent 调度；`search semantic` 使用本地可重建派生向量索引，真实 LLM judge scores 只作为本地/手动评估证据保存，不进入必跑 CI。进入后续产品切片前，仍需保持以下追溯关系：
